...
function lf
    ...
    command find . -maxdepth 1 -type f -a ! -iname ".*" -print0 | sed -e "s:./::g" | xargs -0r ls $param $argv
end
# list hidden items
function lh
    set -l matches .*
...
function lad
    ...
    command find . -maxdepth 1 -type d \( -not -iname "." \) -print0 | sed -e "s:./::g" | xargs -0r ls -dF $param $argv
end
...
function llf
    command find . -maxdepth 1 -type f -a ! -iname ".*" -print0 | sed -e "s:./::g" | xargs -0r ll -hGd $argv
end
# long list all directories
function llad
    command find . -maxdepth 1 -type d \( -not -iname "." \) -print0 | sed -e "s:./::g" | xargs -0r ll -hGd $argv
end
# long list all files + hidden
function llaf
    command find . -maxdepth 1 -type f -print0 | sed -e "s:./::g" | xargs -0r ll -hGd $argv
end
# long list only hidden files
function llhf
    command find . -maxdepth 1 -type f -a -iname ".*" -print0 | sed -e "s:./::g" | xargs -0r ll -hGgd $argv
end
...
if __my_shell_has du
    if command du --version >/dev/null 2>&1
        # disk usage
        function du
            command du -k -d 1 -- $argv 2>/dev/null
        end
        # disk usage
        function du.
            command du -k -d 0 -- $argv 2>/dev/null
        end
    else
        # disk usage
        function du
...
if __my_shell_has du; and __my_shell_has find; and __my_shell_has awk
    ...
    function dushf
        ...
        find . \
            -path './.git' -prune -o \
            -type f -name '.*' -print0 | while read -lz f
            command du -k "$f"
        end | awk '
      { sum += $1; count++ }
      END {
...
if __my_shell_has du; and __my_shell_has mktemp; and __my_shell_has sort; and __my_shell_has wc; and __my_shell_has awk; and __my_shell_has find; and __my_shell_has tr
...
if __my_shell_has grep
    # search history
    function hg
        command history | command grep $argv
    end
end
# clear history
function clhist
...
function hs
    command history | awk '{CMD[$2]++;count++;}END { for (a in CMD)print CMD[a] " " CMD[a]/count*100 "% " a;}' | command grep -v "./" | command column -c3 -s " " -t | command sort -nr | command nl | command head -n10
end
...
if __my_shell_has vm_stat; and __my_shell_has system_profiler; and __my_shell_has grep; and __my_shell_has awk; and __my_shell_has sed
...
if __my_shell_has grc
    # ping with count
    function ping
        command grc ping -c 10 $argv
    end
    # process status
    function ps
        command grc ps aux $argv
    end
else
    # ping with count
    function ping
...
function psg
    command ps aux | command grep -v grep | command grep -i -e VSZ -e $argv
end
# find process
function pfind
...
function mcd
    command mkdir -p $argv; and command cd $argv
end
# mkdir with parent directories and verbose output
function mkd
    command mkdir -pv $argv; and command cd $argv
end
...
if test "$_myos" = Linux
    ...
    if __my_shell_has port
        ...
        if __my_shell_has brew
            ...
            # update Linux/macports
            function update
                echo "install MacPorts or Homebrew" >&2
                return 127
            end
        end
    end
    ...
    # update Linux/macports
    function update
        echo "unsupported system ($_myos)" >&2
        return 1
    end
end
