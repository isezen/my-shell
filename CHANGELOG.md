...
### Fixed
- **`ll_macos` crashed at script load on stock macOS with `declare: -A: invalid option`.** Apple froze `/bin/bash` at 3.2.57 (the last GPLv2 release); any Mac without Homebrew/MacPorts bash in PATH runs every `#!/usr/bin/env bash` script under bash 3.2, which predates associative arrays. `ll_macos` had three `declare -A` top-level declarations (`row_index_map`, `dir_start_index`, `dir_end_index`), so the script died before parsing any argv — every Apple-default macOS user would have been broken. My own machine hid this behind MacPorts bash 5.3 and CI hid it behind the macos-latest runner's brew-provided bash. Caught on my wife's iMac (Monterey, stock bash 3.2 only). Fix: replace the three associative maps with parallel indexed arrays (`row_paths`) plus two small linear-scan helpers (`_find_row_index_by_path`, `_find_dir_slice`) that set globals on hit to avoid subshell forks in the render loop. Lookups are now O(N) per query with tiny N (one listing's operand count), dominated in total cost by `stat` and `awk`, not the shell-side scan. Baseline parity is preserved byte-for-byte. Regression locked by a new `tests/ll_macos/10_core.bats` case that explicitly invokes `ll_macos` via `/bin/bash` (bypassing the shebang's env-resolved bash) and asserts no `declare: -A` error and meaningful output across all three code paths that used the maps. Areas: ll/drivers, bash-compat, tests (138 → 139 passing).
- **`ll_linux` / `ll_macos` flag parser mis-decomposed unknown long options as short flags.** The `-*)` branch in both drivers iterated every character after the leading `-`, which was correct for combined short flags (e.g. `-hGg`) but also captured long options like `--group-directories-first` whose body happens to contain `d`, `g`, `G`, `h`, `n`, `o`, `s`. The `d` character silently set `LISTDIRECTORY=1`, collapsing the entire listing to a single `.` row — reproducible the moment a user typed `ll` in fish, because `shell/fish/aliases.fish` wraps the external binary with `command ll -hGg --group-directories-first $argv`. Fix: add a `--*) ;;` no-op case immediately before `-*)` in both drivers so any unrecognized long option is ignored by our parser (on Linux it still passes through to GNU `ls` via `"$@"`, which does honor `--group-directories-first`; on macOS `stat`-based `ll_macos` has no equivalent and now silently drops it, which is the same contract as every other GNU-only flag). Regression locked by a new `tests/ll/30_driver_flag_parity.bats` case that runs both drivers with `--group-directories-first` against a three-file scratch dir and asserts all three filenames appear in the output. `make baseline-check` still passes byte-for-byte. Areas: ll/drivers, tests (137 → 138 passing).
- **`install.sh` was missing `ll_linux`, `ll_macos`, `ll_common.awk`, and `ll_linux.awk` from its install list.** The `SCRIPTS="ll dus dusf dusf."` string predated the Phase 0 split of `ll` into a platform dispatcher plus per-OS drivers and the Phase 1 extraction of the shared `ll_common.awk` render layer — it was never updated. A fresh install would have placed only the thin `ll` wrapper into the bin directory; on first invocation the dispatcher would fail with `selector=uname target not executable` because `ll_linux` / `ll_macos` never landed, and even if they had, both drivers resolve `ll_common.awk` via `"$(dirname "$0")/ll_common.awk"` at runtime and would `awk: can't open file` without it. Fix: `SCRIPTS` now lists `ll ll_linux ll_macos dus dusf dusf.` (executables), and a new `AWK_LIBS="ll_common.awk ll_linux.awk"` loop installs the two runtime-sourced libraries as mode-0644 data files alongside the drivers (`fetch_or_copy` already accepted a per-file mode). Two new `tests/install.bats` cases lock the invariant — one asserts all six executables are present and `-x`, the other asserts both awk libraries are present, `-f`, and explicitly NOT `-x`. `README.md`'s uninstall command was also stale and is updated to match. Areas: install/packaging, tests (35 → 35 passing, 2 assertions expanded + 1 new test).
